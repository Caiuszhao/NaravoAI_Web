        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
          {STACK.map((el, idx) => {
            ...
            return (
              ...
              >
                {/* Accent gradient */}
                <div className={`absolute inset-0 bg-gradient-to-br ${el.color} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none`} />
                ...
                <div className="relative z-10 flex items-start gap-4">
                  <div className="relative w-12 h-12 shrink-0 rounded-[14px] bg-black border border-white/10 flex items-center justify-center shadow-[0_0_20px_rgba(0,0,0,0.8)] group-hover:border-white/20 transition-all">
                    <Icon className={`w-5 h-5 transition-colors duration-300 ${el.iconColor}`} strokeWidth={1.5} />
                    <div className="absolute inset-0 rounded-[14px] bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none" />
                  </div>
                  ...
                  <div className="flex flex-col gap-2 flex-1 min-w-0">
                    ...
                    <span className="text-[13px] md:text-[14px] font-light text-white/35 leading-relaxed group-hover:text-white/55 transition-colors">
                      {el.desc}
                    </span>
                  </div>
                </div>
                ...
                <div className="relative z-10 h-px bg-white/5 rounded-full overflow-hidden">
                  <div className="h-full w-0 group-hover:w-full bg-gradient-to-r from-white/20 to-transparent transition-all duration-700 ease-out" />
                </div>
              </motion.div>
            );
          })}
